...
| 2 | **Idiomas** | **Seis locales activos: ES · EN · FR · DE · IT · PT.** ⚠️ **DESTINO CAMBIADO POR EL DUEÑO EL 24-AGO-2026: los cuatro idiomas nuevos, TRADUCIDOS AL COMPLETO** («una traducción completa y efectiva sin errores de toda la página en todos los idiomas»). Queda **derogado** el arranque por núcleo de ~30 páginas: se traducen las **310 piezas** de cada idioma (83 artículos + 186 fichas + 41 eventos vigentes ≈ 169.000 palabras). Los eventos **caducados no se traducen**. El despliegue sigue siendo `rollout: 'partial'`, pero solo como **red de seguridad mientras se traduce**: el selector únicamente ofrece el idioma donde la página ya existe, así que nunca hay huérfanas. Orden de ejecución: DE (banco de pruebas) → los cuatro en paralelo. ⚠️ **Neerlandés**: es el mercado con MÁS demanda desaprovechada (5.518 impresiones, posición 6,5, CTR 0,31 %), por encima de los cuatro elegidos; queda fuera por decisión expresa del dueño — **no volver a proponerlo salvo que él lo pregunte**. Ver `docs/superpowers/specs/2026-08-24-idiomas-europa-completo-design.md` y `docs/ESTRATEGIA-MULTIIDIOMA.md` | Se añade o se retira un idioma |
...
| 11 | **Traducción (motor)** | **DECIDIDO 24-ago-2026: IA con glosario de marca + revisión humana.** Traduce Claude aplicando el glosario y la voz de lujo tranquilo (no traducción literal: slugs y metadatos **nativos por mercado**), y todo nace en `status: draft` con `translationStatus: 'auto-sin-revisar'` hasta que una persona lo aprueba. NO se contrata DeepL ni traductor externo por ahora; si la calidad de un mercado lo pide, se añade revisor nativo encima de este flujo (no lo sustituye). Ficha KAN-51 | Un mercado exige revisión nativa profesional |
...
- **Excepción registrada (redes sociales vía Metricool), 2026-07-30.** Por decisión expresa del dueño («lanza las publicaciones también tú desde Metricool, yo no acepto los borradores»), las piezas de redes (`REDES-SOCIALES/`: reels, carruseles, stories, pines) se **programan directamente con auto-publicación** en Metricool — el paso "borrador que aprueba el dueño" se ELIMINA porque en la práctica los borradores quedaban sin aprobar y no se publicaba nada (los reels del 27 y 28-jul-2026 perdieron su fecha así). La aprobación humana se cumple: (a) con los **gates de producción** ya aprobados una vez (gate "solo Menorca" con assets verificados y gate "la foto enseña de lo que habla"), y (b) con el **derecho de veto**: Cristian puede borrar/editar cualquier pieza en Metricool antes de su hora programada. NO afecta a la web (artículos, lugares, traducciones, branded content), que sigue la regla absoluta de `draft` → aprobación → `published`.
...
- 🚨 **TODO LO QUE SE CREE NACE EN LOS SEIS IDIOMAS.** Regla del dueño, 24-ago-2026: *«que a partir de ahora todo lo que se cree sea en todos los idiomas»*. Un artículo, una ficha de lugar o un evento nuevo se crea **a la vez** en ES, EN, FR, DE, IT y PT, unidos por su `translationKey`. No se "deja pendiente" — eso es lo que dejó el francés en el 5 % durante dos meses. Lo verifica `npm run paridad`, que corre en CI y **falla** si una pieza nueva no tiene sus seis versiones (lo que ya existía cuando se puso la regla vive en `scripts/data/paridad-baseline.json` y no rompe la build mientras se traduce). Voz y glosario obligatorios: `docs/GLOSARIO-TRADUCCION.md`.
- **Añadir un idioma NO es "site.ts + diccionario".** Esa promesa estaba escrita aquí y en `site.ts`, y era falsa: son **~330 cadenas** de andamiaje antes de traducir una línea de contenido, más ~24 páginas fijas. Lo que sí es cierto es que **el compilador te da la lista completa**: amplía el tipo `Locale` en `src/config/site.ts` y `astro check` enumera, uno a uno, todos los sitios que faltan por rellenar. Nunca dejes un `locale === "es" ? … : inglés`: eso cae a otro idioma **sin avisar**. Usa un registro por idioma, o declara los idiomas del componente con `Extract<Locale, …>` si la pieza es de despliegue por mercados.
